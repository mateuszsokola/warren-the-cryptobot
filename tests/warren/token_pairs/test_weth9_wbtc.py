...
@pytest.mark.asyncio
async def test_weth9_wbtc(orderbook: OrderBookService):
...
@pytest.mark.asyncio
async def test_wbtc_weth9(orderbook: OrderBookService):
    weth9 = WEth9(web3=orderbook.web3, transaction_service=orderbook.transaction_service)
    amount_in = int(2 * 10**18)
    await weth9.deposit(amount_in=amount_in)
    await weth9.approve(uniswap_v3_router_address, max_amount_in=amount_in)
    ...
    uniswap_v3_wbtc_weth9_pair = create_token_pair(
        async_web3=orderbook.async_web3,
        web3=orderbook.web3,
        transaction_service=orderbook.transaction_service,
    ...
    await uniswap_v3_wbtc_weth9_pair.swap(amount_in=wbtc_balance)

    (wbtc_balance, weth9_balance) = uniswap_v3_wbtc_weth9_pair.balances()
